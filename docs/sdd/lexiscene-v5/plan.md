...
## 平台能力探底(2026-09-01 实测)

| 探测项 | 结果 | 结论 |
|---|---|---|
| git 仓库 / 工作区 | `git rev-parse --git-dir` 通过;工作区干净 | worktree 可用 |
| 原生 worktree 隔离入口 | 当前平台无一键隔离参数,但 git 原生 worktree 可用(Bash 可建) | 用 git 原生,无降级 |
| 并发子 agent | 支持(一条消息并行派多个 Agent) | 每条线一个子 agent 真并行 |
| 后台长任务 | 支持(run_in_background) | 测试/构建不堵线 |
