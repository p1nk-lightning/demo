...
import { Archive, Check, ExternalLink, FileText, RotateCcw, Sparkles } from 'lucide-react';
          ...
          {selected && (
            <article className="rounded-lg border border-ink-200 bg-white p-5 sm:p-7">
              <div className="flex flex-col justify-between gap-4 border-b border-ink-200 pb-5 sm:flex-row sm:items-start">
                ...
                {(selected.status === 'approved' || selected.status === 'published') && <button type="button" disabled={saving} onClick={() => void review('candidate')} title="退回候选" className="icon-button"><RotateCcw size={17} /></button>}
              ...
              {selected.status === 'candidate' && <label className="mt-5 block max-w-xs"><span className="field-label">人工确认后进入文章池，轮换任务会自动发布</span><input type="date" value={publishDate} onChange={(event) => setPublishDate(event.target.value)} className="field-control" /></label>}
